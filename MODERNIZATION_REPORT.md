...
-   **COM**: The Component Object Model is used for OLE embedding (e.g., `COleTemplateServer` in `wordpad.cpp`). While powerful, COM is complex and has been largely superseded by other interoperability technologies.
...
-   **Language**:
    ...
    -   Replace C-style strings and `CString` with **`std::string`** and `std::string_view`.
